...
#
# Intro
stage_intro = utils.StageIntro(screen, config)
user_wears_glasses = stage_intro.loop()
# Webcam
stage_webcam = utils.StageWebcam(screen, webcam, config)
stage_webcam.loop()
# Game
data = Data(
...
data.new_game()
stage_game = utils.StageGame(screen, webcam, data, config)
...
print("HITS PER SECOND: {}".format(len(scores)/float(config.GAME_TIME)))
